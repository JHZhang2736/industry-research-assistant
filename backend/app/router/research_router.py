...
class ResearchRequest(BaseModel):
    ...
    search_web: Optional[bool] = None  # 兼容旧字段
    search_local: Optional[bool] = None  # 兼容旧字段
    search_modes: Optional[list] = None  # 搜索模式: ['web', 'local']
...
async def stream_research(request: ResearchRequest):
    """
    深度研究接口 - 流式输出（DeepResearch V2 多智能体架构）

    使用 Server-Sent Events (SSE) 格式流式返回研究过程和结果。
    """
    search_web = request.get_search_web()
    search_local = request.get_search_local()
    logger.info(f"DeepResearch V2 query: {request.query[:50]}... (session_id: {request.session_id}, search_web={search_web}, search_local={search_local})")
...
async def stream_research_get(
    ...
    """
    深度研究接口 - GET 方式流式输出（DeepResearch V2 多智能体架构）
    """
    logger.info(f"DeepResearch V2 (GET) query: {query[:50]}...")
    service_v2 = get_research_service_v2()
